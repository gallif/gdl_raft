...
from core.modules.admm_v2_0 import ADMMSolverBlock, ADMMInitiator
...
class RAFT(nn.Module):
    def __init__(self, args):
        ...
        if args.admm_solver:
            self.admm_init = ADMMInitiator(admm_mask=args.admm_mask, 
                shape=[sh // 8 for sh in args.image_size]+[int(np.ceil(args.batch_size/torch.cuda.device_count()))], rho=args.admm_rho)
            self.admm_block = ADMMSolverBlock(shape=[sh // 8 for sh in args.image_size]+[int(np.ceil(args.batch_size/torch.cuda.device_count()))], 
                rho=args.admm_rho, lamb=args.admm_lamb, eta=args.admm_eta, T=args.admm_iters)
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, upsample=True):
        ...
        # Generate matrices for ADMM
        if self.args.admm_solver:
            admm_mats,masks = self.admm_init(image1)
        else:
            admm_mats = None
            masks = None
        ...
        for itr in range(iters):
            ...
            # Apply ADMM Solver
            if self.args.admm_solver:
                Q = self.admm_block(delta_flow, admm_mats)
                q.append(Q)
            else:
                Q = 0.0
            
            # F(t+1) = F(t) + \Delta(t) + Q
            coords1 = coords1 + delta_flow + Q
        ...
        return flow_predictions,(q,masks),delta_flows
